...
- **两种内容模式**：
  - **瞬间/说说**：Issue 加 `moment` 或 `瞬间` 标签 → 首页时间轴直接显示正文，无标题，像 QQ 空间说说
  - **长文章**：不带该标签 → 当作普通文章，首页显示标题 + 摘要，点进去看全文
- **时间轴首页**：左侧竖线贯穿，头像在线上，按时间倒序展示所有动态
- **写作方式不变**：继续在 GitHub Issues 里写东西，零迁移成本
- **V2EX 风格**：经典灰白配色、卡片式布局、简约排版
- **完全免费**：GitHub Actions + Cloudflare Pages 都对公开仓库免费
- **全自动同步**：新建/编辑 Issue 后，1-2 分钟内自动更新到网站
...
1. 去 Issues 页面新建 Issue
2. **标题随便写**（不会显示在网站上，只是给你自己识别用）
3. **正文写你想记录的感受**（一两句话也行）
4. **Labels 加上 `moment` 或 `瞬间`**（关键！这决定它显示为说说样式）
5. 可以再加其他标签，比如 `生活`、`编程`、`日常`
6. 提交，等 1-2 分钟，首页时间轴就能看到

### 写一篇「长文章」

1. 去 Issues 页面新建 Issue
2. **标题就是文章标题**
3. **正文就是文章内容**（支持完整 Markdown）
4. **Labels 加文章标签**（**不要**加 `moment`/`瞬间`）
5. 提交，等 1-2 分钟，首页显示标题卡片，点进去看全文

### 草稿箱

给 Issue 加 `draft` 标签，就不会同步到网站。写完了去掉这个标签即可。
